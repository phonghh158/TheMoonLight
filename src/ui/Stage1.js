...
class Stage1UI {
    ...
    jumpToStep(step) {
        ...
        this.logic.step = step;

        // Reset giao diện về trạng thái sạch
        this.dockRods.classList.add("hidden");
        this.dockString.classList.add("hidden");
        this.dockColors.classList.add("hidden");
        this.dockStickers.classList.add("hidden");
        this.starsContainer.classList.add("hidden");
        this.paperContainer.classList.add("hidden");
        this.cutsceneBox.classList.add("hidden");

        // Nếu nhảy đến step 1 hoặc 2 hoặc 3: cần hiển thị 2 khung ngôi sao
        if (step <= 3) {
            this.starsContainer.classList.remove("hidden");
            this.starFrame1.classList.remove("merge-center-left");
            this.starFrame2.classList.remove("merge-center-right");
        }

        // Giả lập lắp đầy đủ các thanh nan nếu step >= 2
        if (step >= 2) {
            [1, 2].forEach((starIndex) => {
                const targetLayer = starIndex === 1 ? this.rodsLayer1 : this.rodsLayer2;
                const starBox = targetLayer.parentElement;

                this.rodConfig.forEach((rodData) => {
                    if (!this.logic[`star${starIndex}Rods`].includes(rodData.name)) {
                        this.logic[`star${starIndex}Rods`].push(rodData.name);
                    }

                    const dashed = starBox.querySelector(`line.dashed-${rodData.name}`);
                    if (dashed) dashed.style.display = "none";

                    if (!targetLayer.querySelector(`[data-placed="${rodData.name}"]`)) {
                        const placedImg = document.createElement("img");
                        placedImg.src = rodData.raw;
                        placedImg.className = "placed-rod-img";
                        placedImg.dataset.placed = rodData.name;
                        placedImg.style.left = rodData.box.left;
                        placedImg.style.top = rodData.box.top;
                        placedImg.style.width = rodData.box.width;
                        placedImg.style.height = rodData.box.height;
        ...
        // Giả lập buộc dây đầy đủ nếu step >= 3
        if (step >= 3) {
            [1, 2].forEach((starIndex) => {
                const jointsLayer = starIndex === 1 ? this.jointsLayer1 : this.jointsLayer2;
                this.joints.forEach((j) => {
                    const list =
                        starIndex === 1
                            ? this.logic.star1TiedJoints
                            : this.logic.star2TiedJoints;
                    if (!list.includes(j.id)) list.push(j.id);

                    const node = jointsLayer.querySelector(`[data-joint="${j.id}"]`);
                    if (node) node.classList.add("tied");
                });
            });
        }

        // Giả lập nối xong 2 khung và dán màu mặc định nếu step >= 5
        if (step >= 5) {
            this.logic.connectedTips = ["top", "right", "bottom_right", "bottom_left", "left"];

            this.logic.paperColors.front[1] = "pink";
            this.logic.paperColors.front[2] = "green";
            this.logic.paperColors.front[3] = "purple";
            this.logic.paperColors.front[4] = "purple";
            this.logic.paperColors.front[5] = "green";
            this.logic.paperColors.front[6] = "gold";

            this.logic.paperColors.back[1] = "pink";
            this.logic.paperColors.back[2] = "green";
            this.logic.paperColors.back[3] = "purple";
            this.logic.paperColors.back[4] = "purple";
            this.logic.paperColors.back[5] = "green";
            this.logic.paperColors.back[6] = "pink";
        }

        // Kích hoạt setup của step tương ứng
        switch (step) {
            case 1:
                this.triggerGuide(this.stepInstructions[1]);
                this.dockRods.classList.remove("hidden");
                break;
            case 2:
                this.setupStep2();
                break;
